...
def _run_synthetic(args: argparse.Namespace) -> None:
    ...
    x = np.zeros((args.batch_size, cfg.max_segments, 1, cfg.n_mels, cfg.seg_length), dtype=np.float32)
    n_wins = np.full((args.batch_size,), min(64, cfg.max_segments), dtype=np.int32)
...
def _run_end_to_end(args: argparse.Namespace, paths: list[Path]) -> None:
    ...
    if args.preprocess_workers == 1 or len(chunks) < 2:
        for chunk in chunks:
        ...
        with ThreadPoolExecutor(max_workers=args.preprocess_workers) as executor:
            futures = [
                executor.submit(_preprocess_one, model, path, channel=args.channel)
