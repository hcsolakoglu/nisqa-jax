...
Use a fresh virtual environment for each backend. Installing the CPU and CUDA
requirements into the same environment can leave the CPU-only `jaxlib` wheel
selected.
...
> out of scope; `requirements-jax.txt` and `requirements-gpu.txt` record the
> tested direct CPU and CUDA 12 dependency versions. Transitive packages,
> including NVIDIA CUDA runtimes, remain resolver-managed; record the complete
> resolved environment with any release benchmark.
...
model in one process; a conflicting later directory is rejected because JAX
0.4.x may keep writing to its first internal cache. The cache directory is
treated as trusted executable code by JAX: it must
